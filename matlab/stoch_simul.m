    ...
    for i=irf_shocks_indx
        if max(abs(cs(:,i))) > 1e-7
            ...
            if options_.nograph == 0
                ...
                elseif nbplt == 1
                    if options_.relative_irf
                        hh = dyn_figure(options_,'Name',[IRF_type, 'Relative response to' ...
                                            ' orthogonalized shock to ' tit(i,:)]);
                    else
                        hh = dyn_figure(options_,'Name',[IRF_type, 'Orthogonalized shock to' ...
                                            ' ' tit(i,:)]);
                    end
                    ...
                    dyn_saveas(hh,[M_.fname '_',deblank(IRF_save_title),'_' deblank(tit(i,:))],options_);
                    if TeX && any(strcmp('eps',cellstr(options_.graph_format)))
                    ...
                    for fig = 1:nbplt-1
                        if options_.relative_irf
                            hh = dyn_figure(options_,'Name',[IRF_type, 'Relative response to orthogonalized shock' ...
                                                ' to ' tit(i,:) ' figure ' int2str(fig)]);
                        else
                            hh = dyn_figure(options_,'Name',[IRF_type, 'Orthogonalized shock to ' tit(i,:) ...
                                                ' figure ' int2str(fig)]);
                        end
                        for plt = 1:nstar
                            subplot(nr,nc,plt);
                        ...
                        dyn_saveas(hh,[ M_.fname '_',deblank(IRF_save_title),'_' deblank(tit(i,:)) int2str(fig)],options_);
                        if TeX && any(strcmp('eps',cellstr(options_.graph_format)))
                            fprintf(fidTeX,'\\begin{figure}[H]\n');
                            for j = 1:nstar
                    ...
                    hh = dyn_figure(options_,'Name',[IRF_type, 'Orthogonalized shock to ' tit(i,:) ' figure ' int2str(nbplt) '.']);
                    m = 0;
                    ...
                    dyn_saveas(hh,[ M_.fname '_',deblank(IRF_save_title),'_' deblank(tit(i,:)) int2str(nbplt) ],options_);
                    if TeX && any(strcmp('eps',cellstr(options_.graph_format)))
                        fprintf(fidTeX,'\\begin{figure}[H]\n');
                        for j = 1:m
