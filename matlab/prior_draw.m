...
persistent uniform_index gaussian_index gamma_index beta_index inverse_gamma_1_index inverse_gamma_2_index dirichlet_index weibull_index
persistent uniform_draws gaussian_draws gamma_draws beta_draws inverse_gamma_1_draws inverse_gamma_2_draws dirichlet_draws weibull_draws
...
if nargin>0 && init
    ...
    dirichlet_index = find(prior_shape==7);
    if isempty(dirichlet_index)
        dirichlet_draws = 0;
    else
        dirichlet_draws = 1;
    end
...
if weibull_draws
    ...
    while ~isempty(out_of_bound),
        pdraw(weibull_index(out_of_bound)) = gamrnd(p6(weibull_index(out_of_bound)),p7(weibull_index(out_of_bound)))+p3(weibull_index(out_of_bound));
        out_of_bound = find( (pdraw(weibull_index)'>ub(weibull_index)) | (pdraw(weibull_index)'<lb(weibull_index)));
    end
end
