...
if ~isempty(options_.nk) && options_.nk ~= 0
    if ~((any(bayestopt_.pshape > 0) && options_.mh_replic) || (any(bayestopt_.pshape> 0) && options_.load_mh_file)) %no Bayesian estimation
        if ~(options_.selected_variables_only && ~(options_.forecast > 0)) %happens only when selected_variables_only is not used
            oo_.FilteredVariablesKStepAhead(:,oo_.dr.inv_order_var(bayestopt_.smoother_saved_var_list),:)=oo_.FilteredVariablesKStepAhead;
            if ~isempty(PK) %get K-step ahead variances
                oo_.FilteredVariablesKStepAheadVariances(:,oo_.dr.inv_order_var(bayestopt_.smoother_saved_var_list),oo_.dr.inv_order_var(bayestopt_.smoother_saved_var_list),:)=oo_.FilteredVariablesKStepAheadVariances;
            end
            if ~isempty(decomp)
                oo_.FilteredVariablesShockDecomposition(:,oo_.dr.inv_order_var(bayestopt_.smoother_saved_var_list),:,:)=oo_.FilteredVariablesShockDecomposition;
            end
        else
